...
void Grammar() // Nyelvtan meghatározása
{
	// Változók: szabályok, bal és jobb oldalai
	vector<string> left_rules;
	vector<string> right_rules;
	string leftSide;
	string rightSide;

	// Szabályok inicializálása
	...
	for (size_t i = 0; i < rules.size(); i++) // Szabályok szétszedése két oldalra
	{
	...
	table(left_rules, right_rules); 	// A megkapott szabályokat a table metódus kapja
}


void table(vector<string> left_rules, vector<string> right_rules) // Tábla megjelenítése
{
	...
	for (int n = 0; n < right_variables.size(); n++) // Jobb oldal megjelenítése
	{
	...
	for(vector<char>::iterator j = right_variables.begin(); j < right_variables.end(); ++j) // Felső sor szabályainak kiíratása
	{
		cout << *j << "\t";
	}
	...
	for(vector<char>::iterator k = variables.begin(); k < variables.end(); ++k)	// Bal oldalon levő oszlop szabályainak kiíratása
	{
		cout << *k << "\t";
...
vector<char> right_table(vector<string> left_value) // Jobb oldali tábla bal oldali értékeinek vizsgálata
{
	vector<char> data;
...
vector<char> left_table(vector<string> right_value) // Bal oldali táblában levő jobb oldali értékek vizsgálása
{
	vector<char> data;
...
int value(char ch)	// Visszaadja a jelenlegi karaktert
{
	int value = ch;
	return value;
}
...
int main()
{
	//Az input.txt fájlból kimásolható több szabály is
	Grammar();
}